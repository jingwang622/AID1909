...
>>* 配置文件：/etc/mysql
>>* 命令集： /usr/bin
>>* 数据库存储目录 ：/var/lib/mysql(拥有非常高的保密权限)
...
>数字类型：
>>整数类型（精确值） - INTEGER，INT(常用)，SMALLINT(常用)，TINYINT(常用)，MEDIUMINT，BIGINT
>>定点类型（精确值） - DECIMAL(常用)
>>浮点类型（近似值） - FLOAT(常用)，DOUBLE
>>比特值类型 - BIT
...
```
e.g.
select * from class1 order by age desc,score;(按照年龄降序，年龄相同，按照分数升序)
```
...
```
e.g.
select * from class1 where sex = 'm' limit 3;
update class1 set score=95 where age = 17 limit 1;
# 查找成绩最好的女生
select * from class1 where sex = 'f' order by score desc limit 1;
```
...
UNION 操作符语法格式(可以看作是一种或关系，但是UNION的效率能高点，还会有比or更多的功能)：
...
[WHERE conditions];
```
>expression1, expression2, ... expression_n: 要检索的列。(列数相同即可，不一定一样，以地一个列为准)
>tables: 要检索的数据表。
...
e.g.
# (列数相同即可，不一定一样，以地一个列为准)
select name,age from class1 where sex = 'm' UNION ALL select name,hobby from interest;
+-------+------------+
| name  | age        |
+-------+------------+
| Tom   | 17         |
...
```
select count(attack),sum(attack) from sanguo;
+---------------+-------------+
| count(attack) | sum(attack) |
+---------------+-------------+
|            13 |       10340 |
+---------------+-------------+
```
...
+---------+
eg3：除非国家和性别一模一样才会去重
select distinct country,gender from sanguo;
+---------+--------+
| country | gender |
+---------+--------+
| 魏      | f      |
...
| age       | tinyint(4)        | YES  |     | 0       |                |
| sex       | enum('m','w','o') | YES  |     | o       |                |
| salary    | decimal(8,2)      | YES  |     | 250.00  |                |
...
+----+--------+------+------+---------+------------+---------+
|  1 | 小明   |   34 | m    | 3456.90 | 2019-02-03 |    NULL |
|  2 | 小红   |   23 | w    | 2000.00 | 2015-03-03 |    NULL |
|  3 | 小李   |   33 | m    | 7000.00 | 2017-03-03 |       2 |
|  4 | 小花   |   25 | w    | 6800.00 | 2015-03-09 |       4 |
|  5 | 小兰   |   25 | w    | 9000.00 | 2014-05-21 |       5 |
|  7 | 小草   |   27 | w    | 2000.00 | 2019-03-21 |       5 |
+----+--------+------+------+---------+------------+---------+
select person.name,dept.dname from person left join dept on person.dept_id = dept.id;
+--------+-----------+
| name   | dname     |
+--------+-----------+
| 小李   | 培训部    |
...
|  8 | 技术部    |
+----+-----------+
select person.name,dept.dname from person right join dept on person.dept_id = dept.id;
+--------+-----------+
| name   | dname     |
+--------+-----------+
| 小李   | 培训部    |
...
> 视图是存储的查询语句,当调用的时候,产生结果集,视图充当的是虚拟表的角色。其实视图可以理解为一个表或多个表中导出来的表，作用和真实表一样，包含一系列带有行和列的数据 视图中，用户可以使用SELECT语句查询数据，也可以使用INSERT，UPDATE，DELETE修改记录，视图可以使用户操作方便，并保障数据库系统安全。(优点：部分表数据呈现给其他人，复杂的select封装 缺点：写操作有制约，查询效率低)

### 视图操作(跟原表的链接，原表名修改，视图不可用)
...
* 存储过程三个参数的区别
  ...
  * 设置变量方法：   set  @[变量名] = 值； 表示这是一个用户变量，使用时用@[变量名]。 在函数内部设置declare [变量名] [变量类型]为局部变量，局部变量可以使用set赋值或者着使用into关键字。
    ...
    ```
    create function f3() returns int begin declare a int; set a = (select age from class1 where name = "Jone"); return a; end$$
    ```
  
    ```
    create function f5() returns int begin declare a int; select age into a from class1 where name = "Jone"; return a; end$$
    ```
...
```
show {shpw|function} status [like’存储过程或存储函数的名称’](很少用)
```
...
```
show create  {procedure|function}  存储过程或存储函数的名称(很少用)
```
...
> 一个事务所做的修改在最终提交以前，对其他事务是不可见的。多个事务相互独立。(存在多种问题)
...
```
all privileges 、select 、insert、index、view、drop ... ... 
库.表 ： *.* 代表所有库的所有表
```
