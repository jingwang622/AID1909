...
* groupdict()  获取捕获组字典，组名为键，对应内容为值（常用）
...
* group(n = 0)（常用）
	...
	# print(m.lastindex) # 最后一组序列号
	
	# 属性方法
	print(m.span()) # 匹配到的内容在目标字符串中的起止位置 ×
	print(m.start())
	print(m.end())
	print(m.groupdict()) # 捕获组组名和对应匹配内容的字典 ×
	print(m.groups()) # 获取所有子组对应的内容
	
	print(m.group()) # 获取match对应内容  ×
	print(m.group('pig'))
	```
...
利用正则表达式完成：
    ...
    编写函数完成，从标准输入输入一个端口名称，返回该端口运行情况描述中的
    address值（address is/Internet address is）

    温馨提示： 先确定是那一段内容，再用正则提取
               每段第一个词是端口名称
...
1. 匹配邮箱地址
   ...
   re.search(r'\w+@\w+(\.com)?(\.cn)?',"abc@123.com.cn").group()

2. 匹配数字
   12  -133  1.5  -4.7  45.8%   1/2

  re.findall(r'-?\d+\.?/?\d*%?',"12  -133  1.5  -4.7  45.8%   1/2")

3. 匹配一段文字中书名
   《xxx》  《啊-大海》  《哈姆雷特》

   re.findall(r'《.+?》',"《xxx》  《啊-大海》  《哈姆雷特》")
